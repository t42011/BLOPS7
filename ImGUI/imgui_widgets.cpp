...
bool ImGui::Keybind(const char* label, int* key, int* mode, bool enablemode)
{
    ...
    // Safe key name lookup
    const char* active_key = (*key >= 0 && *key < IM_ARRAYSIZE(keys)) ? keys[*key] : "Unknown";
    ...
    if (it_anim == anim.end()) { anim[id] = key_state(); it_anim = anim.find(id); }

    ImVec4& color = it_anim->second.text;
    ImVec4 target = g.ActiveId == id ? ImGui::GetStyleColorVec4(ImGuiCol_TextSelectedBg) :
        hovered ? ImGui::GetStyleColorVec4(ImGuiCol_TextDisabled) :
        ImGui::GetStyleColorVec4(ImGuiCol_Text);
    color = ImLerp(color, target, io.DeltaTime * 6.f);

    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, ImGui::GetColorU32(ImGuiCol_FrameBg));
    window->DrawList->AddText(rect.Min, ImGui::ColorConvertFloat4ToU32(color), label);
    PushStyleColor(ImGuiCol_Text, ImGui::GetStyleColorVec4(ImGuiCol_Text));
    ...
    if (g.ActiveId == id)
    {
        ...
        {
            if (!io.MouseDown[0]) waitForMouseRelease[id] = true;
        }
        else
        {
            for (int i = 0; i < 5; i++)
        ...
        if (IsKeyPressedMap(ImGuiKey_Escape)) { *key = 0; ImGui::ClearActiveID(); }
        else *key = k;
    }
